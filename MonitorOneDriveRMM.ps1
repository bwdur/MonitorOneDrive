...
$event_id_success = 1
$dll_url = 'https://raw.githubusercontent.com/rodneyviana/ODSyncService/master/Binaries/PowerShell/OneDriveLib.dll'
$doc_url = "your_doc_url"
$results_path = "C:\programdata\Microsoft OneDrive\OneDriveLogging.txt"
        ...
        if(Test-Path $results_path){
            ...
            foreach ($result in $results) {
                if($result.StatusString -notin $good_results -and $result.ServiceType -eq "Business1"){
                    ...
                    $success_message = "OneDrive is syncing fine!  `n  Status: " + $result.StatusString + "`n  Username: " + $result.UserName + "`n  Service Type: " + $result.ServiceType
                    Write-EventLog -Source $event_source -LogName "Application" -EventID $event_id_success -EntryType Information -Message $success_message
                }
            }
        }
        else{
